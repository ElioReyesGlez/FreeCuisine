...
public class CustomLineView extends View {
    ...
    public static final int SHOW_CURRENT_ONLY = 4;
    ...
    private final ArrayList<ArrayList<Dot>> drawDotLists = new ArrayList<>();
    ...
    @Override protected void onDraw(Canvas canvas) {
        ...
        for (int k = 0; k < drawDotLists.size(); k++) {
            ...
            for (Dot d : drawDotLists.get(k)) {
                ...
                } else if (showPopupType == SHOW_CURRENT_ONLY) {
                    int currentDay = TimeHelper.getCurrentDayOfWeekInNumber();
                    if (d.linenumber == currentDay) {
                        drawPopup(canvas, d.data, d.setupPoint(tmpPoint),
                                colorArray[k % colorArray.length]);
                    }
                }
    ...
    private Dot findPointAt(int x, int y) {
        ...
        for (ArrayList<Dot> data : drawDotLists) {
            for (Dot dot : data) {
                ...
                r.set(pointX - width, pointY - width,
                        pointX + width, pointY + width);
                if (r.contains(x, y)) {
                    return dot;
                }
            }
